...
class segtree
{
    ...
    void stabbing(const Node* n, const unsigned int &query, std::multimap<unsigned int, unsigned int> &result, unsigned int &count)
    {
        ...
        if (n->key > query)  // visit left child case
        {
            ...
            if (n->left != NULL)
            {
                if (n->left->left_most <= query && query <= n->left->right_most) stabbing(n->left, query, result, count);
            }
        }
        else if (n->key <= query)    // visit right child case
        {
            ...
            if (n->right != NULL) 
            {
                if (n->right->left_most <= query && query <= n->right->right_most) stabbing(n->right, query, result, count);
            }
        }
    }
...
public:
    ...
    void stabbing_query(const unsigned int query)
    {
        ...
        stabbing(root, query, result, count);
